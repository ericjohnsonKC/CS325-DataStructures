...
class binaryTree{
    public:
        ...
        void inorderTraversal(void (*visit) (T &item), T searchItem) const;
    ...
    private:
        ...
        void inorder(node<T> *p, void (*visit) (T &item), T searchItem) const;
...
template <class T>
void binaryTree<T>::inorderTraversal(void (*visit) (T &item), T searchItem) const {
    inorder(root, *visit, searchItem);
}
...
template <class T>
void binaryTree<T>::inorder(node<T> *p, void (*visit) (T &item), T searchItem) const {
    if(p != nullptr){
        inorder(p->lLink, *visit, searchItem);
        if(p->data == searchItem){
            (*visit)(p->data);
        }
        inorder(p->rLink, *visit, searchItem);
    }
}
